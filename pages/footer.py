...
def key_hook(explorer, key, mode):
    if key == explorer.settings.keys["/"] and mode == Modes.default:
        explorer.mode = Modes.search
        explorer.regenerate_sections()
    if key == explorer.settings.keys.escape and mode == Modes.search:
    ...
    elif mode == Modes.search and key == explorer.settings.keys.backspace and explorer.entry_index > 0:
        explorer.entry = explorer.entry[:explorer.entry_index - 1] + explorer.entry[explorer.entry_index:]
        explorer.entry_index -= 1
    elif mode == Modes.search and key == explorer.settings.keys.arrow_right:
        explorer.entry_index = min(explorer.entry_index + 1, len(explorer.entry))
    elif mode == Modes.search and key == explorer.settings.keys.arrow_left:
        explorer.entry_index = max(explorer.entry_index - 1, 0)
    elif mode == Modes.search and key == explorer.settings.keys.delete and explorer.entry_index < len(explorer.entry):
